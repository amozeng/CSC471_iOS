...
class ViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        for (index, dic) in favChannels {
            for(name, channelNum) in dic {
                favChannel.setTitle(name, forSegmentAt: index)
            }
        }
    ...
    @IBAction func favChannelSelected(_ sender: UISegmentedControl) {
        ...
        channel.text = String(Array(favChannels[index]!.values)[0])
